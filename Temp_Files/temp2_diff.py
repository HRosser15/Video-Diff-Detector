...
def main():
    ...
    while True:
        ...
        if contours in detected_contours_list:
            frame_count += 1

            # Add the current set of contours to the list
            detected_contours_list.append(contours)
            ...
            if (frame_count >= duration_threshold_frames) and (len(detected_contours_list) > 0):
                # print("Detected contours for the last 5 seconds")
                visualize_difference(frame1, frame2, diff_image, detected_contours_list[-1], len(detected_contours_list[-1]), frame_count)

        elif frame_count > 0:
            # print(f"difference #{difference_count} has ended and should be visualizing now...")
            visualize_difference(frame1, frame2, diff_image, detected_contours_list[-1], len(detected_contours_list[-1]), frame_count)

            # Clear the detected contours list and reset frame_count
            detected_contours_list = []
            frame_count = 0
