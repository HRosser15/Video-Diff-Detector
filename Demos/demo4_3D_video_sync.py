...
def find_frame_difference(frame1, frame2):
    ...
    diff_resized = rescaleFrame(diff, 1)
    cv.imshow('diff', diff_resized)
    cv.waitKey(10)

    # Create a binary image, setting pixels above a threshold to white and below to black
    # The second argument is the threshold, and can be modified 1-254.
    # A lower threshold will show smaller differences
    # A higher threshold will only show significant differences
    _, thresholded_diff = cv.threshold(diff, 50, 255, cv.THRESH_BINARY)
    thresholded_diff_resized = rescaleFrame(thresholded_diff, 1)
...
def main():
    ...
    first_frame_resized = rescaleFrame(first_frame, 1)
    ...
    sync_frame_resized = rescaleFrame(sync_frame, 1)
    ...
    base_vid.set(cv.CAP_PROP_POS_FRAMES, 0)
    alt_vid.set(cv.CAP_PROP_POS_FRAMES, 32)
    ...
    while True:
        ...
        concatenated_frame_resized = rescaleFrame(concatenated_frame, 0.75)
