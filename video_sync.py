...
def main():
    ...
    '''We can add in a conditional here to handle the option to play the videos synced.
    IF TRUE (if option is present in CLI), then play the videos synced, 
        and proceed difference detection either when the videos end or 'q' is pressed'''
    print(base_start_frame)
    print(alt_start_frame)
    return base_start_frame, alt_start_frame
    ...
    while True:
        ...
        cv.putText(frame_base, video1_path, (10, frame_base.shape[0] - 20), cv.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 0, 0), 1)
        cv.putText(frame_alt, video2_path, (10, frame_alt.shape[0] - 20), cv.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 0, 0), 1)
