...
def frame_difference_dct(frame1, frame2, threshold=25, block_size=8):
    # Convert frames to grayscale for simplicity
    gray1 = cv.cvtColor(frame1, cv.COLOR_BGR2GRAY)
    gray2 = cv.cvtColor(frame2, cv.COLOR_BGR2GRAY)

    # Convert grayscale frames to floating-point format
    gray1_float = np.float32(gray1)
    gray2_float = np.float32(gray2)

    # Apply DCT transformation to the entire frame
    dct1 = cv.dct(gray1_float)
    dct2 = cv.dct(gray2_float)

    # Calculate pixel-wise differences in the DCT-transformed frames
    diff_dct = np.abs(dct1 - dct2)

    # Threshold the differences
    _, thresholded_diff = cv.threshold(diff_dct, threshold, 255, cv.THRESH_BINARY)

    # Convert thresholded_diff to 8-bit unsigned integer format
...
def visualize_difference(frame1, frame2, diff_image, contours, diff_value, duration):
    ...
    cv.putText(frame1_with_border, "Video 1", (100, frame1_with_border.shape[0] - 20), cv.FONT_HERSHEY_COMPLEX_SMALL, 1,
               (0, 0, 0), 1)
    cv.putText(frame2_with_border, "Video 2", (100, frame2_with_border.shape[0] - 20), cv.FONT_HERSHEY_COMPLEX_SMALL, 1,
               (0, 0, 0), 1)
...
def main():
    ...
    while True:
        ...
        # Perform pixel-wise difference using DCT and get contours
        diff_image, contours = frame_difference_dct(frame1, frame2)
