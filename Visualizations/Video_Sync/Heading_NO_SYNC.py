...
def main():
    ...
    base_vid.set(cv.CAP_PROP_POS_FRAMES, 243)
    alt_vid.set(cv.CAP_PROP_POS_FRAMES, 210)

    # Define border sizes and output video properties
    border_size = 10
    bottom_border_size = 34
    ...
    text_x_pos = int(video_width / 2 - 40)
    text_y_pos = int(output_height - 13)

    codec = cv.VideoWriter_fourcc(*'mp4v')
    output_video = cv.VideoWriter('Videos/Heading_unsynched.mp4', codec, fps, (output_width, output_height), True)
    ...
    while True:
        ...
        cv.putText(bordered_frame_base, "Video 1", (text_x_pos, text_y_pos), cv.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 0, 0), 1)
        cv.putText(bordered_frame_alt, "Video 2", (text_x_pos, text_y_pos), cv.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 0, 0), 1)
