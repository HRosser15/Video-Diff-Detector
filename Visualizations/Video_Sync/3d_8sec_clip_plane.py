...
def find_matching_frame(gray1, frame2):
    ...
    # Create a binary image, setting pixels above a threshold to white and below to black
    _, thresholded_diff = cv.threshold(diff, 115, 255, cv.THRESH_BINARY)
...
def main():
    ...
    base_vid = cv.VideoCapture('../../Videos/3D_Cockpit_4.mp4')
    alt_vid = cv.VideoCapture('../../Videos/3D_Cockpit_3.mp4')
    ...
    '''MAKE THE CHANGES TO BORDERSIZE, OUTPUT WIDTH, OUTPUT HEIGHt, 
    TEXT SIZE, TEXT WEIGHT, and BOTTOM BORDER SIZE ON OTHER FILE'''
    text_size = 1
    text_weight = 1
    ...
    elif output_width < 720:
        text_size = 2
        text_weight = 2

    elif output_width < 1080:
        text_size = 3
        text_weight = 2
    ...
    if frame_scale <= 0.25:
        text_size = round(text_size / 2, 1)
        text_weight = int(round(text_weight / 2, 0))

    elif frame_scale <= 0.5:
        text_size = round(text_size / 1.5, 1)
        text_weight = int(round(text_weight / 1.5, 0))

    elif frame_scale <= 0.75:
        text_size = round(text_size / 1.25, 1)
        text_weight = int(round(text_weight / 1.25, 0))
    ...
    text_y_pos = int(((output_height) / frame_scale) - bottom_border_size / 2) + 35

    codec = cv.VideoWriter_fourcc(*'mp4v')
    output_video = cv.VideoWriter('Videos/3D_plane_synced.mp4', codec, fps, (output_width, output_height), True)
    ...
    while True:
        ...
        cv.putText(bordered_frame_base, "Video 1", (text_x_pos, text_y_pos), cv.FONT_HERSHEY_COMPLEX_SMALL, 5, (0, 0, 0), text_weight)
        cv.putText(bordered_frame_alt, "Video 2", (text_x_pos, text_y_pos), cv.FONT_HERSHEY_COMPLEX_SMALL, 5, (0, 0, 0), text_weight)
