...
def dct_hash(image, hash_size=8):
    ...
    # Calculate the pHash using DCT method
...
def main():
    ...
    # Convert the first frame to grayscale (if not already)
    if len(first_frame.shape) > 2 and first_frame.shape[2] == 3:
    ...
    # Convert the sync frame to grayscale (if not already)
    if len(sync_frame.shape) > 2 and sync_frame.shape[2] == 3:
