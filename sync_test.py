...
class TestFindFrameDifference(unittest.TestCase):
    ...
    def test_threshold_difference_below_edge(self):
        ...
        self.assertTrue(find_frame_difference(frame1, frame2), 
        "Failed: The difference is right at the threshold.")

    # test the difference that just goes over the threshold    
    def test_threshold_difference_over_threshold(self):
        frame1 = np.zeros((100, 100), dtype=np.uint8)
        frame2 = np.zeros((100, 100, 3), dtype=np.uint8)
        frame2[:50, :50, :] = 255
        frame2[50, 50, :] = 255
        self.assertTrue(find_frame_difference(frame1, frame2), 
        "Failed: The difference is slightly over the threshold.")

    # test for no frame input 
    def test_no_input_frame2(self):
        frame1 = np.zeros((100, 100), dtype=np.uint8)
        with self.assertRaises(TypeError):
            find_frame_difference(frame1)

    # tests invalid data type on frame 1
    def test_invalid_data_frame1(self):
    ...
    def test_invalid_data_frame2(self):
        frame1 = np.zeros((100, 100), dtype=np.uint8)
    ...
    def test_large_frame_difference(self):
        ...
        self.assertTrue(find_frame_difference(frame1, frame2), "Failed. Frame difference too big.")
